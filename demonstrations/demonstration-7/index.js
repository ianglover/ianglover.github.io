...
const googleDriveTableUrl =
  "https://api.airtable.com/v0/appNBOMaj7ch0wKcM/Google-Drive-Illustrations";
const tableUrl = "https://api.airtable.com/v0/appNBOMaj7ch0wKcM/Illustrations";

// URL with API Key Authentication
const googleDriveAuthenticatedUrl =
  googleDriveTableUrl + "?api_key=" + airtableApiKey;
...
// Get a 'Promise' Representing the Data that Will be Returned from the Google Drive URL
fetch(googleDriveAuthenticatedUrl)
  // When 'Promise' is 'Resolved', get the JSON data
  .then((res) => res.json())
  // Use the JSON Data
  .then((data) => {
    console.log("Google Drive Table Data: ", data.records);
  })
  .catch((error) => {
    console.log(error);
  });
...
fetch(authenticatedUrl)
  ...
  .then((data) => {
    ...
    console.log("Illustration Table Data: ", illustrations);
